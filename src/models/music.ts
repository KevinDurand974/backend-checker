...
export const updateOneMusic = async (updatedData: Partial<Music>, id: number) => {
  try {
    const [[musicDb]] = (await connection.query('SELECT * FROM musics WHERE music_id = ?', [id])) as RowDataPacket[][];
...
export const removeOneMusicById = async (id: number) => {
  try {
    ...
    const [del] = (await connection.query('DELETE FROM musics WHERE music_id = ?', [id])) as RowDataPacket[];
